...
Calculate the call chain.

chain->item->|

Find out possible types of item:
Find all instances that starts with the chain in its Search Grouping, and note all following method call names
Ask the repo for all types that have all of these methods

If none found, shift off the leftmost item and check again (this time without a root item) until a type is identified.

If none found, match all types.
...
Return hard coded list of methods and docs. Is it quick enough?

Repo tables for CPAN and Project dbs.

Test data for a few tables

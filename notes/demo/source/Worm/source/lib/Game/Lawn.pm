...
Game::App object used to steer the game, or undef if there is no app.

Default: undef

=cut

use Class::MethodMaker get_set => [ "oApp" ];
...
sub prizeWasClaimedBy { my $self = shift;
    ...
    $self->oApp and ($self->oApp->prizeWasClaimedBy($oPrize, $oObject) or return(0));
...
sub placeObjectBodyPartAt { my $self = shift;
    ...
    $self->rhGrid->{$oLocation->left}->{$oLocation->top}->{$oObject} = $oObject;
    $self->oUI->displayObjectBodyPartAt($oLocation, $char);
...
sub wormHasCrashed { my $self = shift;
    ...
    $self->oApp and ($self->oApp->wormHasCrashed($oObject) or return(0));
